...
//	set_non_blocking(listen_fd);
    ...
    while (!exit_accept_flag) {
        ...
        snprintf(log_str_buf, LOG_STR_BUF_LEN,
                 "Epoll event[%d] *****connected from %s*****, fd:%d, current connect total = %d.\n",
...
int main(){
    ...
    // create fork.. not good for clion to debug
    pid_t pidfd = fork();
    if (pidfd < 0) {
        printf(" fork fail!!! \n");
        return (-1);
    ...
    if (pidfd != 0) {
        printf("parent can exit now\n");
        exit(0);
    }

    setsid();// leave the parent process
        ...
        for (index = 0; index < epoll_events_number; ++index) // deal with the event
        {
            ...
            if (events[index].events & EPOLLIN) //have read event
            {
                ...
                if (0 == recv_buffer_from_fd(connect_socket_fd_temp, recv_buffer, &recv_length)) {
                    snprintf(log_str_buf, LOG_STR_BUF_LEN,
                             "recv_length = %d, current fd = %d, current job queue job number = %d.\n", recv_length,
                             connect_socket_fd_temp, get_jobqueue_number(thpool));
                    LOG_INFO(LOG_LEVEL_INDISPENSABLE, log_str_buf);
